...
import RayonsTable from '../containers/ranking/RayonsTable';
import AddRayon from '../components/models/AddRayon';
...
const RankingRayons = () => {
  ...
  const { rayonModal, showActions, cupboard } = useSelector((state) => state.ranking);
      ...
      <div className="w-full bg-slate-200 p-2 h-screen">
        <div className="my-3 flex flex-row justify-between xs:mx-1 md:mx-4">
          ...
          <div className="relative">
            ...
            >
              <Actions list={actions?.filter(action => action !== 'New Cupboard')} />
            </div>
          </div>
        </div>
        <RayonsTable />
        <div className={`${rayonModal ? 'flex' : 'hidden'}`}>
          <AddRayon isOpen={rayonModal} cupboard={cupboard} />
        </div>
      </div>
    </Layout>
  );
