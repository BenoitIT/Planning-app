            ...
            {page.map((row) => {
              ...
              return (
                ...
                >
                  {row.cells.map((cell) => {
                    if (
                      cell.column?.Header === 'Entry' &&
                      cell?.value?.length > 5000
                    ) {
                      return (
                        <td
                          key={cell.id}
...
function GlobalFilter({
  ...
  return (
    <label className="relative max-w-[50%] min-w-[30%]">
      ...
      />
      <Button
        className="absolute right-4 top-1/2 transform -translate-y-1/2 !bg-transparent !border-none !ring-none !shadow-none hover:!text-primary !p-0"
